function LootFilter.OnEvent()
	...
	if (event == "CHAT_MSG_LOOT") and (LootFilterVars[LootFilter.REALMPLAYER].enabled) then
	...
	if (event == "UNIT_INVENTORY_CHANGED") and (LootFilterVars[LootFilter.REALMPLAYER].enabled) then
		DEFAULT_CHAT_FRAME:AddMessage("Loot Filter - UNIT_INVENTORY_CHANGED");

		for bag=0,NUM_BAG_SLOTS do
		   for slot=1,GetContainerNumSlots(bag) do
			  --item = GetContainerItemID(bag,slot)
			  --print(item)
			  local item = LootFilter.getBasicItemInfo(GetContainerItemLink(bag, slot));
				if (item ~= nil) then
					--print("item ok")
					if (not LootFilterVars[LootFilter.REALMPLAYER].caching) then
						--print("item added")
					...
					LootFilter.schedule(5, LootFilter.processItemStack);
				end;
		   end
		end
